#!/bin/bash
#
# Randomized operation log focused on multi-pack-index compatibility.
...
gen_ops() {
    ...
    while [ "$step" -le "$steps" ]; do
        ...
        case "$op" in
            ...
            8)
                if [ "$((step % 2))" -eq 0 ]; then
                    echo "midx-write --bitmap" >> "$out"
                else
                    echo "midx-write" >> "$out"
                fi
                ;;
            9)
                echo "midx-verify" >> "$out"
                ;;
            10)
                echo "midx-expire" >> "$out"
                ;;
        esac
        step=$((step + 1))
    done
    ...
    while read -r op arg1 arg2; do
        case "$op" in
            ...
            repack)
                ...
                ;;
            midx-write)
                if has_pack_file; then
                    if [ -n "$arg1" ]; then
                        "$run_cmd" multi-pack-index write "$arg1"
                    else
                        "$run_cmd" multi-pack-index write
                    fi
                fi
                ;;
            midx-verify)
                if has_pack_file && test -f .git/objects/pack/multi-pack-index; then
                    "$run_cmd" multi-pack-index verify
                fi
                ;;
            midx-expire)
                if has_pack_file && test -f .git/objects/pack/multi-pack-index; then
                    "$run_cmd" multi-pack-index expire
                fi
                ;;
            *)
...
compare_repos() {
    ...
    if test -f "$repo_git/.git/objects/pack/multi-pack-index"; then
        test -f "$repo_bit/.git/objects/pack/multi-pack-index"
    fi
}
...
test_expect_success 'random multi-pack-index ops seed=701' '
    run_case 701 60 4
'

test_expect_success 'random multi-pack-index ops seed=702' '
    run_case 702 60 4
'
